...
class CenterNet(nn.Module):
    ...
    def forward(self, images, features_dict, gt_instances):
        ...
        if not self.training:
            ...
            if self.more_pos:
                ...
                pos_inds, labels = self._add_more_pos(reg_pred, gt_instances, shapes_per_level)

            losses = self.losses(
                pos_inds, labels, reg_targets, flattened_hms, logits_pred, reg_pred, agn_hm_pred
            )
    ...
    def _get_ground_truth(self, grids, shapes_per_level, gt_instances):
        ...
        strides = torch.cat(
            [shapes_per_level.new_ones(num_loc_list[l]) * self.strides[l] for l in range(L)]
        ).float()  # M
        ...
        for i in range(len(gt_instances)):  # images
            ...
            is_peak = ((grids.view(M, 1, 2).expand(M, N, 2) - centers_discret) ** 2).sum(
                dim=2
            ) == 0  # M x N
    ...
    def _get_label_inds(self, gt_instances, shapes_per_level):
        ...
        for im_i in range(B):
            ...
            label = (
                targets_per_im.gt_classes.view(n, 1).expand(n, L)[is_cared_in_the_level].view(-1)
            )
    ...
    def get_center3x3(self, locations, centers, strides):
        ...
        return (dist_x <= strides_expanded[:, :, 0]) & (dist_y <= strides_expanded[:, :, 0])
    ...
    def inference(self, images, clss_per_level, reg_pred_per_level, agn_hm_pred_per_level, grids):
        ...
        if self.only_proposal:
            ...
            proposals = self.predict_instances(
                grids, logits_pred, reg_pred_per_level, images.image_sizes, agn_hm_pred_per_level
            )
    ...
    def _add_more_pos(self, reg_pred, gt_instances, shapes_per_level):
        ...
        if N > 0:
            with torch.no_grad():
                c33_reg_loss = (
                    self.iou_loss(
                        reg_pred_c33.view(N * L * K, 4), c33_regs_expand, None, reduction="none"
                    )
                    .view(N, L, K)
                    .detach()
                )  # N x L x K
